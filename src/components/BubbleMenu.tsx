...
export function BubbleMenu({
	...
	const extensionsNames = editor.extensionManager.extensions.map(
		(ext) => ext.name,
	);

	const renderMenuItems = () => [
		extensionsNames.includes("columns") && !bubbleMenu?.columnConfig?.hidden ? (
			<ColumnsBubbleMenu key="columns" editor={editor} />
		) : null,
		extensionsNames.includes("table") && !bubbleMenu?.tableConfig?.hidden ? (
			<TableBubbleMenu key="table" editor={editor} />
		) : null,
		extensionsNames.includes("link") && !bubbleMenu?.linkConfig?.hidden ? (
		...
		// ) : null,
		!bubbleMenu?.floatingMenuConfig?.hidden ? (
			<ContentMenu key="content" editor={editor} disabled={disabled} />
		) : null,
